...
class Serializer:
    def __init__(self, b: bytes):
        self.msg_format = int.from_bytes(b, byteorder="big")
...
class Broker:
    ...
    def subscribe(self, topic: str, address: socket.socket, _format: Serializer = None):
        ...
        if topic["value"] is not None:
            ...
            byt = _format.serialize(send_msg)
            address.sendall(byt)
    ...
    def read(self, conn: socket.socket):
        ...
        if _format:
            ...
            msg = serializer.deserialize(msg_bytes)
            method = msg["method"]
            if method == "SUBSCRIBE":
    ...
    def publicate(self, msg: Dict):
        ...
        for (addr, s) in topic["consumers"]:
            ...
            if msg_serialized[msg_format] is None:
                msg_serialized[msg_format] = s.serialize(msg_to_send)
            addr.sendall(msg_serialized[msg_format])
